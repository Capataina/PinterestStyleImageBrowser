...
mod tests {
    ...
    fn test_inference_speed() {
        ...
        let test_image = std::path::Path::new(
            "test_images/66bb951dcab9c27a144292c8_WestStudio-LOL-Splash-Vol2-021.jpg",
        );
    ...
    fn test_batch_preprocess_image() {
        ...
        let batch_size = 4;
        let batches = encoder
            .batch_preprocess_image(&image_paths, batch_size)
            .unwrap();
        assert_eq!(batches.len(), 1);
        assert_eq!(batches[0].shape(), &[4, 3, 224, 224]);
    }
    ...
    fn test_encode_multiple_images() {
        ...
        assert!(
            !embeddings.is_empty(),
            "Should successfully encode at least one image"
        );
    ...
    fn test_encode_batch() {
        ...
        // Encode individually
        let individual: Vec<Vec<f32>> = paths.iter().map(|p| encoder.encode(p).unwrap()).collect();
        ...
        for (batch_emb, indiv_emb) in batched.iter().zip(individual.iter()) {
            ...
            for (b, i) in batch_emb.iter().zip(indiv_emb.iter()) {
                assert!(
                    (b - i).abs() < 1e-5,
                    "Batch and individual encoding should match"
                );
            }
        }
